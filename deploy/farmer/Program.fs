...
let main argv =
    let (storageName, webAppName, rgName, domain) =
        ...
        | Some "test" ->
            "activegamenighttest",
            "active-game-night-test",
            "activegamenight-test",
            "active-game-night-test.azurewebsites.net"
        | Some "prod" ->
            "activegamenight", "active-game-night", "activegamenight", "active-game-night.azurewebsites.net"
        | _ -> failwith "invalid deployEnvironment"

    let azureAppId =
        Environment.GetEnvironmentVariable("AGN_AZURE_APPID")

    let azureSecret =
        Environment.GetEnvironmentVariable("AGN_AZURE_SECRET")

    let azureTenant =
        Environment.GetEnvironmentVariable("AGN_AZURE_TENANT")
